...
func TestAgentMetricsBroadcastDedupesBindMountDiskTotals(t *testing.T) {
	s, _ := store.New(":memory:")
	defer s.Close()
	...
	for {
		...
		if data["disk_used"] != float64(91) {
			t.Fatalf("expected de-duped disk_used 91, got %#v", data["disk_used"])
		}
		if data["disk_total"] != float64(201) {
			t.Fatalf("expected de-duped disk_total 201, got %#v", data["disk_total"])
		}
		break
	}
}
