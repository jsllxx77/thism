...
func TestAgentReleaseManifestPrefersArtifactVersionFile(t *testing.T) {
	originalVersion := sharedversion.Version
	sharedversion.Version = "v1.2.3"
	t.Cleanup(func() {
		sharedversion.Version = originalVersion
	...
	if err := os.WriteFile(filepath.Join(distDir, "thism-agent-linux-amd64.version"), []byte("v9.9.9\n"), 0o644); err != nil {
