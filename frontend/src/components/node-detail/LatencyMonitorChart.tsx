...
export function LatencyMonitorChart({ monitors, results, range }: Props) {
  ...
  useEffect(() => {
    setVisibleSeries((current) => {
      ...
      for (const monitor of monitors) {
        next[monitor.id] = current[monitor.id] ?? true
      }
      return next
    })
  }, [monitors])
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {monitors.map((monitor, index) => {
            const active = visibleSeries[monitor.id] ?? true
            return (
              <button
                key={monitor.id}
                type="button"
                aria-pressed={active}
                onClick={() => setVisibleSeries((current) => ({ ...current, [monitor.id]: !active }))}
